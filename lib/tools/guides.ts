...
export const GUIDES: Record<string, ToolGuide> = {
  ...
  jwt: {
    summary: "Both sides are live: the token drives the decoded claims, and with the secret the claims drive the token. Decoding and verification stay separate operations, and neither ever implies the other.",
    points: [
      ...
      { title: "Set the dates without doing the arithmetic", body: "A JWT time claim is epoch seconds, which nobody can read or write by hand. Pick a date, or use a preset like +7d, and the claim is written back as whole seconds — milliseconds there would produce a token that expires tens of thousands of years from now." },
      { title: "Editing the claims re-signs the token", body: "Change the payload and the token above is rebuilt, because signing covers exactly the bytes verification checks. Without the secret the token is left alone rather than drifting silently out of step with what is on screen." },
  ...
  "json-merge": {
    ...
    points: [
      ...
      { title: "Records are matched, not just deduplicated", body: "Two entries describing the same host or user differ in some field by definition, so a plain union keeps both. Auto looks for an identifying field — id, name, alias, url and similar — and merges the records that share one, reporting which field it used." },
      { title: "Only identity-shaped fields count", body: "A field whose values merely happen to be unique, like a counter or a price, is never treated as an identity — matching on one would silently combine unrelated records. Name any field yourself with the by-key strategy." },
